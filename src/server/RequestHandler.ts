...
export class RequestHandler<CONTEXT extends Context = Context, RESULT = void> {
    pipeline: Pipeline<CONTEXT, RESULT> = new Pipeline();
    errorPipeline: Pipeline<ErrorContext<CONTEXT>, any> = new Pipeline();
    viewPipeline: Pipeline<ViewContext<CONTEXT, RESULT>, any> = new Pipeline();
    ...
    callback = async (request: IncomingMessage, response: ServerResponse) => {
        ...
        try {
            let result = await this.pipeline.run(context as any);
            ...
            if (this.errorPipeline.middlewares.length) {
                try {
                    ...
                    let result = await this.errorPipeline.run(errorContext);
                    if (result.type === 'exit') {
                        this.sendError(context, error(result.value, { status: errorStatus }));
                    } else {
                        this.send(context, result);
                    }
                } catch (e) {
                    ...
                    this.sendError(context, error(e, { status: errorStatus }));
                }
            } else {
                this.sendError(context, error(e, { status: errorStatus }));
            }
    ...
    async sendView(context: Context, directive: ViewDirective<RESULT>) {
        ...
        try {
            ...
            let { value } = await this.viewPipeline.run(viewContext);
    ...
    async sendError<T extends Error>(context: Context, directive: ErrorDirective<T>) {
        ...
        try {
            if (this.viewPipeline.middlewares.length && accept && accept.indexOf('text/html') > -1) {
                ...
                await this.sendView(context, view(value.message, { status, header, template: ERROR_TEMPLATE }) as any);
            } else {
    ...
    log(context: Context, status: number = 500) {
    ...
    use<NEW_DATA, NEW_RESULT = RESULT>(
        middleware: Middleware<CONTEXT & Context<NEW_DATA>, NEW_RESULT>
    ...
    use(middleware: Middleware<any, any>): RequestHandler<CONTEXT & RESULT> {
        this.pipeline.use(middleware);
        return this as any;
    }

    useError<NEW_RESULT>(middlware: Middleware<ErrorContext<CONTEXT>, NEW_RESULT>) {
        return this.errorPipeline.use<ErrorContext<CONTEXT>, NEW_RESULT>(middlware);
    }

    useView<NEW_RESULT>(middlware: Middleware<ViewContext<CONTEXT, NEW_RESULT>, NEW_RESULT>) {
